...
When the library is unable to connect to the API, or if the API returns a non-success status code (i.e., 4xx or 5xx response), a subclass of `SDKLimes\Core\Exceptions\APIException` will be thrown:

```php
<?php

use SDKLimes\Core\Exceptions\APIConnectionException;
use SDKLimes\Core\Exceptions\RateLimitException;
use SDKLimes\Core\Exceptions\APIStatusException;
